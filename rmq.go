...
// 注册发送指定队列指定路由的生产者
func (r *RMQ) RegisterProducer(producer Producer) {
	r.producerList = append(r.producerList, producer)
}

// 发送任务
func (r *RMQ) listenProducer(producer Producer) {
	// 验证链接是否正常,否则重新链接
	if r.channel == nil {
		r.mqConnect()
	}
...
// 注册接收指定队列指定路由的数据接收者
func (r *RMQ) RegisterReceiver(receiver Receiver) {
	r.mu.Lock()
...
// 监听接收者接收任务
func (r *RMQ) listenReceiver(receiver Receiver) {
	// 处理结束关闭链接
	defer r.mqClose()
	// 验证链接是否正常
	if r.channel == nil {
